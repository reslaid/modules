...
class Bloody(loader.Module):
    ...
    def handlers(self):
        ...
        async def blstop(event):
            ...
            args_array = await self.get_args(event, maxsplit=1)
            ...
            await event.edit("**Successfully**", parse_mode="markdown")
        ...
        async def blsetfile(message):
            ...
            args_array = await self.get_args(message, maxsplit=1)
            args = " ".join(args_array)
        ...
        async def blfiles(message):
            ...
            if not templates:
                ...
                current_shablon = self.shablon_name or "not selected"
                text = f"<b>Available template files:</b>\n\n{result}\n\n<b>Current template: </b><code>{current_shablon}</code>"

            await message.edit(text, parse_mode='html')

        @self.strict_owner_command
        async def blupload(event, _reply=None):
            """reply: get link to file"""

            reply = await event.get_reply_message() if not _reply else _reply

            if reply:
                if reply.photo:
                    await event.delete()
                    photo = reply.photo
                    filename = await self.client.download_media(photo, "data/cache")
                    url = 'https://telegra.ph/upload'
                    files = {'file': ('image.jpg', open(filename, 'rb'))}
                    ...
                    await event.delete()
                    video = reply.video
                    filename = await self.clients.download_media(video, "data/cache")
                    url = 'https://telegra.ph/upload'
                    files = {'file': ('video.mp4', open(filename, 'rb'))}
                    response = requests.post(url, files=files)
                    if response.status_code != 200:
                        return
                    link = 'https://telegra.ph' + response.json()[0]['src']
            ...
            if not reply:    
                await event.edit("Where is the reply to the message?")
                return None
        ...
        async def blsetvd(event):
            ...
            try:
                ...
                try:
                    response = urlopen(args)
                except:
                    return await event.edit(f"<b>Invalid url:</b> <code>{args}</code>", parse_mode='html')
        
                if os.path.exists(self.video_file):
                    os.remove(self.video_file)
        
                with open(self.video_file, "w", "utf-8") as videofile:
                    videofile.write(args)
        
                self.log.info(f"New video applied: {args}")
                self.video_url = args
        ...
        async def blsetph(event):
            ...
            try:
                ...
                with open(self.photo_file, "w", encoding="utf-8") as ph:
                    ph.write(args)
