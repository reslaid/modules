...
class Ai(Module):
    ...
    async def answer(self, prompt: str) -> str:
        url: str = 'https://chatgpt.apinepdev.workers.dev/?question='
        full_url: str = url + f"prompt={prompt}"
    ...
    def handle(self):
        ...
        async def gentxt(event):
            ...
            await event.edit('**Generating..**', parse_mode='markdown')
            text = await self.answer(request)
            await event.edit('**Generating...**', parse_mode='markdown')
            await event.delete()
            sender = await event.get_sender()
            await event.respond(f"**{sender.first_name}({sender.id}, {'@'+sender.username or 'None'}): ```{request}```**\n\n**XannaxyGPT: \"{text}\"**", parse_mode='markdown')
        ...
        async def xgentxt(event):
            ...
            await event.edit('**Generating.**', parse_mode='markdown')
            await event.edit('**Generating..**', parse_mode='markdown')
            text = await self.xanswer(request)
            await event.edit('**Generating...**', parse_mode='markdown')
            await event.delete()
            sender = await event.get_sender()
            await event.respond(f"**{sender.first_name}({sender.id}, {'@'+sender.username or 'None'}): ```{request}```**\n\n**XannaxyGPT: \"{text}\"**", parse_mode='markdown')
        ...
        async def translate(event):
            ...
            translated = await self.translate(text, args[2], args[1])
            await event.delete()
