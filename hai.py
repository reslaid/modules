...
class Ai(Module):
    ...
    def handle(self):
        ...
        async def gentxt(event):
            ...
            request = ' '.join(event.message.text.split('.gentxt ', maxsplit=1)[1:])
        ...
        async def xgentxt(event):
            ...
            request = ' '.join(event.message.text.split('.xgentxt ', maxsplit=1)[1:])
            await event.edit('**Generating.**', parse_mode='markdown')
            await event.edit('**Generating..**', parse_mode='markdown')
        ...
        async def translate(event):
            ...
            translated = await self.translate(text, to_lang, from_lang)
            await event.delete()
